...
#ifdef VAP_WPS
			else if(!strncmp(next->d_name, "wscd-wlan0-va0-wlan1-va0.pid", strlen("wwscd-wlan0-va0-wlan1-va0.pid")))
			{
				iw_message(MESS_DBG_CONTROL,"next->d_name=%s\n", next->d_name);	

				strcpy(wscd[0].wlanName,"wlan0-va0");
				strcpy(wscd[1].wlanName,"wlan1-va0");
				//printf("AP mode-->> \n\n");
				link_wscd=2;				
			}			
#endif			
