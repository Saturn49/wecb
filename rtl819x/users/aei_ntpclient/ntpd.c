...
#ifdef AEI_WECB
uint32_t set_first_time = 0;  //set this flag if update time success
uint32_t set_time = 0;  //update time success
uint32_t ForceUpdateFrequency = 0; // 0 not use, other need bigger than 16s
uint32_t MinUpdateFrequency = 0; //0 not use, other need bigger than 16s
uint32_t reply_first_time = 0; //set this flag if get reply from NTP server, means there is a NTP server can work
uint32_t timeout_count = 0; //request  timeout number
#define CTLVS_SYNCHRONIZED   "Synchronized"
#define CTLVS_UNSYNCHRONIZED "Unsynchronized"
#define CTLVS_ERROR_FAILEDTOSYNCHRONIZE "Error_FailedToSynchronize"
#define REQUEST_LOOP  2 // should be 2-6, at the beginning if all server request more than this loop num, but can't get a reponse, means NTP servers can't work
int debug = 0;
...
static int  sntpGetDayFromWeekday(struct tm *tmp,int month, int week, int weekday, char *pYear)
{
	...
	snprintf(pYear, 8, "%d%d", century,year);
	if (month==1 || month==2)
	{
		if (year > 0)
		{
...
int get_daylight_saving(time_t newtime)
{
    char *p = NULL;
    char *comma = ",";
    char buf[512], timezone[128], daystart[128], dayend[128];
    int found = 0, daylightsaving = 1;
    int bm = 3, bw = 2, em = 11, ew = 1;
    struct tm *gmt; 
    int mDaylightsaving = 1;
    int mYear, mMonth, mWeekday, mMonthday, mHour, mMinute, mSecond, mInt;
    time_t now;
    int i;
    ...
    if (debug)
    fprintf(stderr, "gmt.tm_year=%d  gmt.tm_mon=%d gmt.tm_mday=%d gmt.tm_wday=%d gmt.tm_hour=%d gmt.tm_min=%d gmt.tm_sec=%d\n",
    gmt->tm_year,gmt->tm_mon,gmt->tm_mday,gmt->tm_wday, gmt->tm_hour, gmt->tm_min, gmt->tm_sec);

    mMonth = gmt->tm_mon;
    mMonthday = gmt->tm_mday;
    mWeekday = gmt->tm_wday;

    if (debug)
    fprintf(stderr, "--mMonth=%d mMonthday=%d mWeekday=%d\n", mMonth, mMonthday, mWeekday);

    if((mMonth < bm) || (mMonth > em))
    {
      mDaylightsaving = 0;
    } 
    else if(mMonth == bm)
    {
      if((mMonthday >= 1) && (((7 * (bw - 1)) >= mMonthday)))
      {
          mDaylightsaving = 0;
      } else if((mMonthday >= (7 * (bw - 1) + 1)) && ((7 * bw) >= mMonthday))
      {
          if((mWeekday >= 1) && (6 >= mWeekday))
          {
              if((mWeekday) >= (mMonthday - (7 * (bw - 1))))
                  mDaylightsaving = 0;
          } else if(mWeekday == 7)
          {
              if(mHour < 2)
    ...
    {
      if((mMonthday >= (7 * (ew - 1 ) + 1)) && ((7 * ew) >= mMonthday))
      {
          if ((mWeekday >= 1) && (6 >= mWeekday))
          {
              if(mWeekday < (mMonthday - (7 * (ew - 1))))
                  mDaylightsaving = 0;
          } else if(mWeekday == 7)
          {
              if(mHour >= 2)
      ...
      } else if(mMonthday > (7 * ew))
      {
          mDaylightsaving = 0;
      }
    }

    //offset += mDaylightsaving * 60 ;

     //return offset;
    fprintf(stderr, "--mDaylightsaving=%d\n", mDaylightsaving);
...
void FAST_FUNC bb_show_usage(void)
{
	...
	full_write2_str("\n	-t	[TimeZone]");
	full_write2_str("\n	-s	daylight saving");
    full_write2_str("\n	-f	force update frequency");
    full_write2_str("\n	-m	minimum update frequency");
#endif    
	full_write2_str("\n	-n	Do not daemonize");
	full_write2_str("\n	-q	Quit after clock is set");
	full_write2_str("\n	-N	Run at high priority");
...
typedef struct {
	...
	msg_t            p_xmt_msg;
#ifdef AEI_WECB
    uint8_t          time_flag; //set time by this NTP server
#endif
} peer_t;
...
enum {
    ...
    OPT_m = (1 << 9),
#endif
    OPT_l = (1 << 10) * ENABLE_FEATURE_NTPD_SERVER,
...
#define PROBE_LOCAL_ADDR /* { len_and_sockaddr lsa; lsa.len = LSA_SIZEOF_SA; getsockname(p->query.fd, &lsa.u.sa, &lsa.len); } */
	...
	if (p->p_fd == -1) {
		...
		fd = xsocket_type(&local_lsa, family, SOCK_DGRAM);
		//AEI check fd
		if(fd<0)
		{
		    bb_error_msg("ntp sleep 10s");
			sleep(10);
			return;
		}
		p->p_fd = fd;
...
step_time(double *offset)  
{
    ...
    //porting from AT&T BGW project
    tvn.tv_sec += get_daylight_saving(tvn.tv_sec) * 3600;
/*
...
*/
	...
	bb_error_msg("Update time");

    if(set_first_time == 0)
    {
       char cmd[256] = {0};
	   snprintf(cmd, sizeof(cmd),"cli -s Device.Time.Status string %s",CTLVS_SYNCHRONIZED);
	   system(cmd);
    }

    set_first_time = 1;
    *offset = (tvn.tv_sec + (1.0e-6 * tvn.tv_usec)) - (tvc.tv_sec + (1.0e-6 * tvc.tv_usec));
...
set_next_and_close_sock:
    ...
    if(set_time)
    {
       //update time success, set next update time
       if(ForceUpdateFrequency > 0 )
       {
          interval = ForceUpdateFrequency;
       }
       else if(MinUpdateFrequency > interval)
       {
          interval = MinUpdateFrequency;
       }
...
static NOINLINE void ntp_init(char **argv)
{
	...
	char* tzname = NULL;
    int i;
    char* forceFrequency = NULL;
    char* minFrequency = NULL;	
#endif    
	srandom(getpid());
	...
	opts = getopt32(argv,
	                "nqNs" /* compat */
	                "wp:S:t:f:m:"IF_FEATURE_NTPD_SERVER("l") /* NOT compat */
	                "d" /* compat */
	                "46aAbgL", /* compat, ignored */
	                &peers, &G.script_name,&tzname,&forceFrequency,&minFrequency,&G.verbose);

    
    if(forceFrequency)
    {
       ForceUpdateFrequency = atoi(forceFrequency);
       if(ForceUpdateFrequency > 0 && ForceUpdateFrequency < 16)
       {
          ForceUpdateFrequency = 16;
       }
    }

    if(minFrequency)
    {
       MinUpdateFrequency = atoi(minFrequency);
       if(MinUpdateFrequency > 0 && MinUpdateFrequency < 16)
       {
          MinUpdateFrequency = 16;
       }
    }
...
#undef G
    ...
    int sleep_time = 1;
    char cmd[256] = {0};
	//try until connected, connected means get NTP servers IP address
	do
	{
	    try_count++;
		memset(&G, 0, sizeof(G));
		...
		if(G.peer_cnt==0)
		{
           if(try_count == 20)
           {
              //can't get NTP servers IP address, set the status to Error_FailedToSynchronize
              snprintf(cmd, sizeof(cmd),"cli -s Device.Time.Status string %s",CTLVS_ERROR_FAILEDTOSYNCHRONIZE);
              system(cmd);
           }
           if(try_count > 20)
           {
              sleep_time = 1*(try_count-20);
              if(sleep_time > 10)
              {
                sleep_time = 10;
              }
...
#ifdef AEI_WECB
                   //start a new query, clear set_time flag
                   if(p->time_flag)
                   {
                      set_time = 0;
                      p->time_flag = 0;
...
#ifdef AEI_WECB      
                    if(reply_first_time == 0)
                    {
                       timeout_count++;
                       if(timeout_count == REQUEST_LOOP*G.peer_cnt)
                       {
                          //can't get a reponse, means NTP servers can't work
                          snprintf(cmd, sizeof(cmd),"cli -s Device.Time.Status string %s",CTLVS_ERROR_FAILEDTOSYNCHRONIZE);
	                      system(cmd);
                       }
                    }
                    ...
                    if(set_time)
                    {
                       //already has one NTP server update time success,
                       //this server request timeout, but no need try again in a short time, try again at next long loop
                       if(ForceUpdateFrequency > 0 )
                       {
                          timeout = ForceUpdateFrequency;
                       }
                       else if(MinUpdateFrequency > timeout)
                       {
                          timeout = MinUpdateFrequency;
                       }
                    }
#endif
				    set_next(p, timeout);
