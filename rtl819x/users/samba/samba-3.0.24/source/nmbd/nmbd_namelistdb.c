...
#ifdef AEI_WECB
static void ignore_dotlocal_in_name( struct nmb_name *n )
{
#define __STR_DOT_LOCAL__   ".LOCAL"

    /* Clear the tailer of '.LOCAL' */
    char *p = strrchr_m(n->name,'.');
    if( p && (0 == StrCaseCmp(p, __STR_DOT_LOCAL__)) ) {
        memset( p, 0, strlen(__STR_DOT_LOCAL__) );
    }
}
#endif
...
#ifdef AEI_WECB
    ignore_dotlocal_in_name( &uc_name );
#endif
