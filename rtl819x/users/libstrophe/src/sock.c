...
sock_t sock_connect(const char * const host, const unsigned int port)
{
    ...
    fprintf(stderr, "before call getaddrinfo host %s\n", host);

    if ((err = getaddrinfo(host, service, &hints, &res)) != 0)
        return -1;

    fprintf(stderr, "end call getaddrinfo host %s\n", host);
    ...
    while (ainfo) {
        ...
        if ((sock = socket(ainfo->ai_family, ainfo->ai_socktype, 
            ...
            fprintf(stderr, "before call connect host %s\n", inet_ntoa(((struct sockaddr_in *)ainfo->ai_addr)->sin_addr) );
            err = connect(sock, ainfo->ai_addr, ainfo->ai_addrlen);
            fprintf(stderr, "end call connect host %s\n", inet_ntoa(((struct sockaddr_in *)ainfo->ai_addr)->sin_addr) );
