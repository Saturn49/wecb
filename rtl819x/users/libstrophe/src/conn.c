...
#ifdef AEI_XMPP
        if (conn->resource) xmpp_free(ctx, conn->resource);
#endif
...
#ifdef AEI_XMPP
/** Get the resource used for authentication of a connection.
 *
 *  @param conn a Strophe connection object
 *
 *  @return a string containing the resource or NULL if it has not been set
 *
 *  @ingroup Connections
 */
const char *xmpp_conn_get_resource(const xmpp_conn_t * const conn)
{
    return conn->resource;
}

/** Set the resource used to authenticate the connection.
 *  If any resource was previously set, it will be discarded.  The function
 *  will make a copy of the resource string.
 *
 *  @param conn a Strophe connection object
 ...
 *  @ingroup Connections
 */
void xmpp_conn_set_resource(xmpp_conn_t * const conn, const char * const resource)
{
    if (conn->resource) xmpp_free(conn->ctx, conn->resource);
    conn->resource = xmpp_strdup(conn->ctx, resource);
}

/** Get the domain used for authentication of a connection.
 *
 *  @param conn a Strophe connection object
 *
 *  @return a string containing the resource or NULL if it has not been set
 ...
 *  @ingroup Connections
 */
const char *xmpp_conn_get_domain(const xmpp_conn_t * const conn)
{
    return conn->domain;
}

/** Set the domain used to authenticate the connection.
 *  If any domain was previously set, it will be discarded.  The function
 *  will make a copy of the domain string.
 *
 *  @param conn a Strophe connection object
 ...
 *  @ingroup Connections
 */
void xmpp_conn_set_domain(xmpp_conn_t * const conn, const char * const domain)
{
    if (conn->domain) xmpp_free(conn->ctx, conn->domain);
    conn->domain = xmpp_strdup(conn->ctx, domain);
}

#endif

const char *xmpp_conn_get_streamid(const xmpp_conn_t * const conn)
{
...
int xmpp_connect_client(xmpp_conn_t * const conn, 
			  ...
			  void * const userdata)
{
    char connectdomain[2048]={0};
    int connectport;
    const char * domain;
#ifdef AEI_XMPP
    static dns_host_t srvs = NULL;
    dns_host_t dnsnext = NULL;
#endif

    conn->type = XMPP_CLIENT;

#ifdef AEI_XMPP
    conn->domain = xmpp_conn_get_domain(conn);
    xmpp_debug(conn->ctx, "xmpp", "connect to domain: %s", conn->domain);
#else
    ...
    if (altdomain) {
        ...
        char srv_name[1024] = {0};
        snprintf(srv_name, sizeof(srv_name), "_xmpp-client._tcp.%s", conn->domain);

        if (! srvs ) { /* not get dns srv data */
            srvs =  dns_resolve(srv_name, DNS_QUERY_TYPE_SRV);

            if (! srvs) { /* DNS query srv failed */
                xmpp_error(conn->ctx, "xmpp", "SRV lookup failed.");
                ...
                while (dnsnext!= NULL) {
                    if (! srv_cnt) { /* get first srv ip and port */
                        snprintf(connectdomain, sizeof(connectdomain), "%s", ((dns_srv_t) dnsnext->rr)->name);
                        connectport = ((dns_srv_t) dnsnext->rr)->port;

                        xmpp_debug(conn->ctx, "xmpp", "[%d] connectdomain=%s, connectport=%d", srv_cnt+1, connectdomain,connectport);
                    }
                    srv_cnt ++;
                    dnsnext = dnsnext->next;
                }
                xmpp_debug(conn->ctx,"xmpp","DNS SRV total count is %d", srv_cnt);
                set_svrip_cnt(srv_cnt);
            }
            ...
            while (dnsnext!= NULL) {
                if (get_svrip_id() == srv_cnt) {
                    snprintf(connectdomain, sizeof(connectdomain), "%s", ((dns_srv_t) dnsnext->rr)->name);
                    connectport = ((dns_srv_t) dnsnext->rr)->port;
                    xmpp_debug(conn->ctx, "xmpp", "get srv connectdomain %s,connectport %d",
                                    connectdomain, connectport);
                    break;
                }
                srv_cnt ++;
                dnsnext = dnsnext->next;
    ...
    xmpp_debug(conn->ctx, "xmpp","DNS-SRV get domain: %s,port %d", connectdomain, connectport);

    conn->sock = sock_connect(connectdomain, connectport);
    xmpp_debug(conn->ctx, "xmpp", "connect sock %d", conn->sock);
			  ...
			  const char * const fmt, ...)
{
    ...
    if (len >= 1024) {
        ...
        xmpp_debug(conn->ctx, "xmpp", "SENT: %s", bigbuf);
                                  ...
                                  void * const userdata)
{
    ...
    if (xmpp_stanza_to_text(stanza, &buf, &len) == 0) {
        xmpp_debug(conn->ctx, "xmpp", "RECV: %s", buf);
//        xmpp_debug(conn->ctx, "xmpp","From: %s", xmpp_stanza_get_attribute(stanza, "from"));
//        xmpp_debug(conn->ctx, "xmpp","To: %s", xmpp_stanza_get_attribute(stanza, "to"));

//        xmpp_debug(conn->ctx, "xmpp","Name: %s", xmpp_stanza_get_name(stanza));
//        xmpp_debug(conn->ctx, "xmpp","Type: %s", xmpp_stanza_get_type(stanza));
