...
static void _auth(xmpp_conn_t * const conn)
{
    ...
    if (conn->tls_support) {
        ...
        xmpp_debug(conn->ctx, "auth", "try xmpp DIGEST-MD5 auth mechanism");
        ...
        xmpp_debug(conn->ctx, "auth", "try xmpp PLAIN auth mechanism");
        ...
        str = xmpp_jid_node(conn->ctx, conn->jid);
#ifdef AEI_XMPP
        xmpp_error(conn->ctx, "auth", "client authentication is %s", str? str:"failed!");
...
#ifdef AEI_XMPP
        resource = xmpp_conn_get_resource(conn);
        if ((resource != NULL) && (strlen(resource) == 0)) {
            resource = xmpp_conn_get_jid(conn);
        }
#else
