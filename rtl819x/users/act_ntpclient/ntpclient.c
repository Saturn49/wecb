...
struct ntp_control {
	...
	int cross_check;
	char serv_addr[4];
	char dayLightSavings[4];
	char dayLightSavingsOffSet[4];
...
static void get_ntp_param(char *pname,char* ret)
{
    char cmd[256];
	FILE *fp;
	do {
	    sprintf(cmd, "uci get %s",pname);	
		fp =  popen(cmd, "r");
		if(fp != NULL)
		{
...
static void  set_daylight(struct ntptime *new,struct ntp_control *ntpc)
{
    if (debug){
        printf("begin set_daylight\n");
    }
	struct tm tm_start;
	struct tm tm_end;
	struct tm *tm_tmp;
	time_t timep_start;
	time_t timep_end;
    time_t timep_current;
    unsigned int coarse_tmp;
    
    if (debug){
        printf("**begin**new->coarse=%u\n",new->coarse);
    }
    coarse_tmp = new->coarse;
    //add timezone
    if (debug){
        printf("timezone=%s\n",ntpc->timezone);
    }
    if(strstr(ntpc->timezone,"GMT")!=NULL){
        char *tmp_tz = NULL;
        char tmp_hour[64]={0};
        tmp_tz = strtok(ntpc->timezone,":");
        printf("tmp_tz=%s\n",tmp_tz);
        printf("tmp_tz+4=%s\n",tmp_tz+4);
        
        if(strstr(tmp_tz,"-")!=NULL){
            strcpy(tmp_hour,tmp_tz+4);
            if (debug){
                printf("*******++++++tmp_hour=%s\n",tmp_hour);
            }
            coarse_tmp += atoi(tmp_hour)*3600;
        }else if(strstr(tmp_tz,"+")!=NULL){
            strcpy(tmp_hour,tmp_tz+4);
            if (debug){
                printf("*******-------tmp_hour=%s\n",tmp_hour);
            }
    ...
    timep_current = (time_t)coarse_tmp-JAN_1970;
    if (debug){
        printf("timep_current=%ld\n",timep_current);
    }
	tm_tmp = gmtime(&timep_current);
    if (debug){
	    printf("current time=%d-%d-%d %d:%d:%d\n",tm_tmp->tm_year+1900,tm_tmp->tm_mon+1,tm_tmp->tm_mday,tm_tmp->tm_hour,tm_tmp->tm_min,tm_tmp->tm_sec);
    }
    if(ntpc->dayLightSavingsStart!=NULL){
        char tmp_start[256]={0};
        sprintf(tmp_start,"%d-%s:00",tm_tmp->tm_year+1900,ntpc->dayLightSavingsStart);
        if (debug){
            printf("tmp_start=%s\n",tmp_start);
        }
        if (!strptime(tmp_start, "%Y-%m-%dT%H:%M:%S", &tm_start)){
            return;
        }
        if (debug){
	...
	if(ntpc->dayLightSavingsEnd!=NULL){
        char tmp_end[256]={0};
        sprintf(tmp_end,"%d-%s:00",tm_tmp->tm_year+1900,ntpc->dayLightSavingsEnd);
        if (debug){
            printf("tmp_end=%s\n",tmp_end);
        }
		if (!strptime(tmp_end, "%Y-%m-%dT%H:%M:%S", &tm_end)){
            return;
        }
        if (debug){
	...
	timep_start = mktime(&tm_start)+JAN_1970;
	timep_end=mktime(&tm_end)+JAN_1970;
    if (debug){
	    printf("start=%u,end=%u,current=%u\n",timep_start,timep_end,coarse_tmp);
    }
	if((timep_start<=(time_t)coarse_tmp)&&(timep_end>=(time_t)coarse_tmp)){
        if (debug){
            printf("set daylight\n");
        }
        new->coarse +=(atoi(ntpc->dayLightSavingsOffSet))*60;
	}
    if (debug){
	    printf("end set_daylight\n");
    }
...
#undef FAIL
	...
	if (ntpc->set_clock) { /* you'd better be root, or ntpclient will crash! */
		//add daylight
#if 0
        get_ntp_param("ntpclient.@ntpclient[0].dayLightSavings",ntpc->dayLightSavings);
        get_ntp_param("ntpclient.@ntpclient[0].dayLightSavingsOffSet",ntpc->dayLightSavingsOffSet);
        get_ntp_param("ntpclient.@ntpclient[0].dayLightSavingsStart",ntpc->dayLightSavingsStart);
        get_ntp_param("ntpclient.@ntpclient[0].dayLightSavingsEnd",ntpc->dayLightSavingsEnd);
        get_ntp_param("system.@system[0].timezone",ntpc->timezone);
        printf("dayLightSavings=%s\n",ntpc->dayLightSavings);
        printf("dayLightSavingsOffSet=%s\n",ntpc->dayLightSavingsOffSet);
        printf("dayLightSavingsStart=%s\n",ntpc->dayLightSavingsStart);
        printf("dayLightSavingsEnd=%s\n",ntpc->dayLightSavingsEnd);
        printf("timezone=%s\n",ntpc->timezone);
#endif
        if(!strcmp(ntpc->dayLightSavings,"1")){
            set_daylight(&xmttime,ntpc);
	}
            set_time(&xmttime);
	}
...
#ifdef ACT_FIRST_USE
			/* save first use time to file /mnt/rt_conf/firstuse */
			struct stat status;
			if(stat(FIRSTUSE, &status) < 0){
				system("date > "FIRSTUSE);
			}
#endif
