...
char *apmib_hwconf(void)
{
	...
	{
	flash_read_conf((char *)&compHeader, HW_SETTING_OFFSET, sizeof(COMPRESS_MIB_HEADER_T));
	if(memcmp(compHeader.signature, COMP_HS_SIGNATURE, COMP_SIGNATURE_LEN) == 0 ) //check whether compress mib data
	{
		...
		if ( (compLen > 0) && (compLen <= HW_SETTING_SECTOR_LEN) ) {
			...
			flash_read_conf((char *)compFile, HW_SETTING_OFFSET, compLen+sizeof(COMPRESS_MIB_HEADER_T));
...
//mib_display_data_content(HW_SETTING, expFile+sizeof(PARAM_HEADER_T), hsHeader.len-1);
	...
	// Read hw setting
{
	if ( flash_read_conf((char *)&hsHeader, HW_SETTING_OFFSET, sizeof(hsHeader))==0 ) {
//		printf("Read hw setting header failed!\n");
		return NULL;
	}
}
...
#endif
	if ( flash_read_conf(buff, HW_SETTING_OFFSET+sizeof(hsHeader), hsHeader.len)==0 ) {
...
//fprintf(stderr,"\r\n __[%s-%u]",__FILE__,__LINE__);
	flash_read_conf((char *)&compHeader, DEFAULT_SETTING_OFFSET, sizeof(COMPRESS_MIB_HEADER_T));
//fprintf(stderr,"\r\n __[%s-%u]",__FILE__,__LINE__);	
	if(memcmp(compHeader.signature, COMP_DS_SIGNATURE, COMP_SIGNATURE_LEN) == 0 ) //check whether compress mib data
	{
		...
		if ( (compLen > 0) && (compLen <= DEFAULT_SETTING_SECTOR_LEN) ) {
			...
			flash_read_conf((char *)compFile, DEFAULT_SETTING_OFFSET, compLen+sizeof(COMPRESS_MIB_HEADER_T));
...
//mib_display_data_content(DEFAULT_SETTING, expFile+sizeof(PARAM_HEADER_T), dsHeader.len-1);
	...
	if ( flash_read_conf((char *)&dsHeader, DEFAULT_SETTING_OFFSET, sizeof(dsHeader))==0 ) {
...
#endif
	if ( flash_read_conf(buff, DEFAULT_SETTING_OFFSET+sizeof(dsHeader), dsHeader.len)==0 ) {
...
int flash_read_raw_mib(unsigned char **compFile)
{
	...
	flash_read_conf(&compHeader, pDst, sizeof(COMPRESS_MIB_HEADER_T));
	if(memcmp(compHeader.signature, COMP_CS_SIGNATURE, COMP_SIGNATURE_LEN) == 0 ) //check whether compress mib data
	{
		...
		if ( (compLen > 0) && (compLen <= CURRENT_SETTING_SECTOR_LEN) ) {
			*compFile=calloc(1,compLen+sizeof(COMPRESS_MIB_HEADER_T));
			...
			flash_read_conf(*compFile, pDst, compLen+sizeof(COMPRESS_MIB_HEADER_T));		
		}
		else 
		{
			COMP_TRACE(stderr,"\r\n ERR:compLen = %u __[%s-%u]",compLen, __FILE__,__LINE__);
...
int flash_write_raw_mib(unsigned char **compFile)
{
	...
	if ( flash_write_conf(*compFile, pDst, compHeader->compLen+sizeof(COMPRESS_MIB_HEADER_T))==0 )
	{
...
//fprintf(stderr,"\r\n __[%s-%u]",__FILE__,__LINE__);
	flash_read_conf((char *)&compHeader, CURRENT_SETTING_OFFSET, sizeof(COMPRESS_MIB_HEADER_T));
//fprintf(stderr,"\r\n __[%s-%u]",__FILE__,__LINE__);	
	if(memcmp(compHeader.signature, COMP_CS_SIGNATURE, COMP_SIGNATURE_LEN) == 0 ) //check whether compress mib data
	{
		zipRate = compHeader.compRate;
		...
		if ( (compLen > 0) && (compLen <= CURRENT_SETTING_SECTOR_LEN) ) {
			...
			flash_read_conf((char *)compFile, CURRENT_SETTING_OFFSET, compLen+sizeof(COMPRESS_MIB_HEADER_T));
...
//mib_display_data_content(CURRENT_SETTING, expFile+sizeof(PARAM_HEADER_T), csHeader.len-1);
	...
	if ( flash_read_conf((char *)&csHeader, CURRENT_SETTING_OFFSET, sizeof(csHeader))==0 ) {
...
#endif
	if ( flash_read_conf(buff, CURRENT_SETTING_OFFSET+sizeof(csHeader), csHeader.len)==0 ) {
...
//mib_display_tlv_content(HW_SETTING, data, hsHeader.len);		
		...
		if(compress_hw_setting && (write_hw_tlv == 0))
			flash_write_conf((char *)&hsHeader,HW_SETTING_OFFSET,sizeof(hsHeader));
		
		if ( flash_write_conf((char *)data, HW_SETTING_OFFSET+sizeof(hsHeader), hsHeader.len)==0 ) {
...
#endif //#ifedf COMPRESS_MIB_SETTING	
		if ( flash_write_conf((char *)data, i, len)==0 ) {
...
//mib_display_tlv_content(CURRENT_SETTING, data, tlv_content_len+1);			
	...
	if ( flash_write_conf((char *)&header, i, sizeof(header))==0 ) {
	...
	if ( flash_write_conf((char *)data, i, header.len)==0 ) {
		printf("Write flash current-setting failed!\n");
#if CONFIG_APMIB_SHARED_MEMORY == 1
		apmib_sem_unlock();
#endif
...
int apmib_updateFlash(CONFIG_DATA_T type, char *data, int len, int force, int ver)
{
	...
	if(0==flash_write_conf(data,offset,len)){
...
int apmib_updateFlash(CONFIG_DATA_T type, char *data, int len, int force, int ver)
{
	...
	if ( flash_write_conf((char *)pHdr, i, sizeof(PARAM_HEADER_T))==0 ) {
		printf("Write flash current-setting header failed!\n");
#if CONFIG_APMIB_SHARED_MEMORY == 1
		apmib_sem_unlock();
#endif
	...
	if ( flash_write_conf(pMibData, i, curLen)==0 ) {
		printf("Write flash current-setting failed!\n");
#if CONFIG_APMIB_SHARED_MEMORY == 1
		apmib_sem_unlock();
#endif
	...
	if (offset > 0) {
		if ( flash_write_conf((char *)ptr, i, offset)==0 ) {
			printf("Write flash current-setting failed!\n");
#if CONFIG_APMIB_SHARED_MEMORY == 1
			apmib_sem_unlock();
#endif
	...
	if ( flash_write_conf((char *)&checksum, i, sizeof(checksum))==0 ) {
...
#endif
/////////////////////////////////////////////////////////////////////////////////
static int flash_read1(char *buf, int offset, int len)
{
	int fh;
	int ok=1;
...
static int flash_read_conf(char *buf, int offset, int len)
{
	...
	fh = open(FLASH_SAVE_CONFIG, O_RDONLY);
	if ( fh == -1 )
		return 0;
...
//	fprintf(stderr,"%s %d\n",__FUNCTION__,__LINE__);
	...
	/*compress*/
	pCompptr = malloc((CONFIG_RTL_CURRENT_SETTING_END-CURRENT_SETTING_OFFSET)+sizeof(COMPRESS_MIB_HEADER_T));
	if(NULL == pCompptr){
...
#ifdef AEI_WECB
/* Actiontec customized code */


static int isEmptyWscPin( unsigned char * pWscPin )
{
    return ((pWscPin == NULL) || (pWscPin[0] == '\0'));

}

// This example code was copied from WiFi SPEC
static int ValidateChecksum(unsigned long int PIN)
{
...
static int isValidWscPin( unsigned char * pWscPin )
{
    int bValid = 0;
    ...
    do {
        ...
        if( !ValidateChecksum(atol(pWscPin))) {
            break;
        }
        bValid = 1;
    } while(0);
...
/* Description:
 *      For old board, WSC PIN was saved at HW_WLAN0_SWC_PIN & WH_WLAN1_WSC_PIN
 *      Now add a new param DEF_WLAN_WSC_PIN to save its default value.
 *      Call this function to check if DEF_WLAN_WSC_PIN was empty, pick up the
 *      value from HW section as its default setting.
 * return value:
 *      0 - success
 *     -1 - fail
 */
...
{
    int ret = -1;
    unsigned char wps_pin[8+1] = {0};

    do {
        /* APMIB init */
        if ( !apmib_init_HW()) {
            printf("Initialize AP MIB HW failed!\n");
            break;
        }
        if ( !apmib_init()) {
        ...
        /* Check DEF_WLAN_WSC_PIN */
        if( !apmib_getDef( MIB_WLAN_WSC_PIN, (void *)wps_pin)) {
            fprintf( stderr, "APMIB get DEF_WLAN_WSC_PIN failed\n" );
            break;
        }
        printf( "DEF_WLAN_WSC_PIN: '%s'\n", wps_pin );
        if( ! isEmptyWscPin(wps_pin) ) {
            printf( "Default WSC PIN already there.\n" );
            break;
        }

        /* Check HW_WSC_PIN */
        memset(wps_pin, 0, sizeof(wps_pin));
        if( !apmib_get( MIB_HW_WSC_PIN, (void *)wps_pin)) {
            fprintf( stderr, "APMIB get HW_WSC_PIN failed\n" );
            break;
        }
        printf( "HW_WSC_PIN: '%s'\n", wps_pin );
        if( ! isValidWscPin(wps_pin) ) {
            printf( "HW_WPS_PIN '%s' is not valid WSC PIN\n", wps_pin );
            break;
        }

        /* When value of DEF_WLAN_WSC_PIN being empty
         * and value of HW_WSC_PIN being valid,
         * copy value of HW_WSC_PIN to DEF_WLAN_WSC_PIN
         */
        if( !apmib_setDef( MIB_WLAN_WSC_PIN, (void *)wps_pin)) {
            fprintf( stderr, "APMIB set DEF_WLAN_WSC_PIN failed\n" );
            break;
        }
        /* Save to falsh */
        if( !apmib_update( DEFAULT_SETTING)) {
            fprintf( stderr, "APMIB update failed\n" );
            break;
        }
...
/* Description:
 *      Call this function to copy the value of DEF_WLAN_WSC_PIN
 *      to HW_WLAN0_WSC_PIN and HW_WLAN1_WSC_PIN.
 * return value:
 *      0 - success
 *     -1 - fail
 */
int act_reset_wsc_pin()
{
    ...
    do {
        ...
        printf( "DEF_WLAN_WSC_PIN: '%s'\n", wps_pin );
        if( ! isValidWscPin(wps_pin) ) {
            fprintf( stderr, "Default WSC PIN was invalid\n" );
            // Still copy to HW section
        }
        /* No matter value of DEF_WLAN_WSC_PIN being valid or not,
         * copy DEF_WLAN_WSC_PIN to HW_WLAN0_WSC_PIN and HW_WLAN1_WSC_PIN.
         */
        wlan_idx = 0; // 5GHz
        vwlan_idx = 1; // va 0
        if( !apmib_set( MIB_HW_WSC_PIN, (void *)wps_pin)) {
            fprintf( stderr, "APMIB set HW_WSC_PIN failed\n" );
            break;
        }
        wlan_idx = 1; // 2.4GHz
        vwlan_idx = 1; // va 0
