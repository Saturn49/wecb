...
#if defined(AEI_WECB)
#define MAX_PIN_FAIL_THRESHOLD_10 10
#endif
...
#if defined(AEI_WECB) && defined (AEI_WPS_CLIENT)
        if(!strcmp(token, "WEP_KEY"))
        {
            continue;
        }
#endif
...
#if defined(AEI_WECB) && defined(AEI_WIFI) && defined(AEI_WECB_CUSTOMER_TELUS)
static void set_mib_ampdu_amsdu_by_aggregation(int aggregation, struct wifi_mib *pmib)
{
    if(pmib)
    {
        if(0 == aggregation){
...
//!CONFIG_RTL_8196B => rtl8651c+rtl8190
	...
	// set band
        ...
        set_mib_ampdu_amsdu_by_aggregation(intVal, pmib);
#endif
#endif
...
#if defined(AEI_WECB)
    intVal = 0;
    apmib_get(MIB_MAX_PIN_FAIL_THRESHOLD, (void *)&intVal);
    if(intVal >= 1 && intVal <= MAX_PIN_FAIL_THRESHOLD_10)
    {
        WRITE_WSC_PARAM(ptr, tmpbuf, "MaxPinFailThresHold = %d\n", intVal);
    }
    else
    {
        WRITE_WSC_PARAM(ptr, tmpbuf, "MaxPinFailThresHold = %d\n", MAX_PIN_FAIL_THRESHOLD_10);
    }

    intVal = 0;
    apmib_get(MIB_PIN_DISABLE_CONFIG_BY_EXREG, (void *)&intVal);
    if(intVal)
    {
