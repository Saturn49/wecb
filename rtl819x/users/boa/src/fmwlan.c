...
#if defined(WIFI_SIMPLE_CONFIG) && defined(VAP_WPS) && defined(AEI_WIFI)
	vwlan_idx = 1;	
	memset(&wps_config_info, 0, sizeof(struct wps_config_info_struct));
	wps_config_info.caller_id = CALLED_FROM_WLANHANDLER;
	apmib_get(MIB_WLAN_SSID, (void *)wps_config_info.ssid); 
...
#if defined(VAP_WPS) && defined(AEI_WIFI)
	char vap_if[80];
	sprintf(vap_if, "wlan%d-va0", wlan_idx);
	apmib_save_wlanIdx();	
	SetWlan_idx(vap_if);
#endif
...
{
#if !defined(VAP_WPS) && defined(AEI_WIFI) //#ifndef VAP_WPS
	int i=0;
	int wlanif_idx = 0;
	char ifname[20];
...
#if defined(CONFIG_RTL_92D_SUPPORT)
#if defined(VAP_WPS) && defined(AEI_WIFI)
		wlanDisabled[0] = pMib->wlan[0][1].wlanDisabled;
		wlanDisabled[1] = pMib->wlan[1][1].wlanDisabled;
		wlanMode[0] = pMib->wlan[0][1].wlanMode;
		wlanMode[1] = pMib->wlan[1][1].wlanMode;		
		apmib_reset_wlan_to_default("wlan0-va0");
		apmib_reset_wlan_to_default("wlan1-va0");
		pMib->wlan[0][1].wlanDisabled = wlanDisabled[0];
		pMib->wlan[1][1].wlanDisabled = wlanDisabled[1];
#else
		wlanif = whichWlanIfIs(PHYBAND_5G);
...
#ifndef NO_ACTION	
#if defined(VAP_WPS) && defined(AEI_WIFI)
				int wlan0_wsc_disable, wlan1_wsc_disable, wlan0_root_disabled, wlan1_root_disabled;
				apmib_save_wlanIdx();	
				SetWlan_idx("wlan0");
				apmib_get(MIB_WLAN_WLAN_DISABLED, &wlan0_root_disabled);
				SetWlan_idx("wlan1");
				apmib_get(MIB_WLAN_WLAN_DISABLED, &wlan1_root_disabled);
				SetWlan_idx("wlan0-va0");				
				apmib_get(MIB_WLAN_WSC_DISABLE, (void *)&wlan0_wsc_disable);
				SetWlan_idx("wlan1-va0");
				apmib_get(MIB_WLAN_WSC_DISABLE, (void *)&wlan1_wsc_disable);
				apmib_recov_wlanIdx();

				if (wlan0_root_disabled == 0 && wlan0_disable == 0 && wlan0_wsc_disable == 0 && 
						wlan1_root_disabled == 0 && wlan1_disable == 0 && wlan1_wsc_disable == 0) 	
					sprintf(tmpbuf, "%s -sig_pbc wlan0-va0-wlan1-va0", _WSC_DAEMON_PROG);				
				else
...
#if defined(VAP_WPS) && defined(AEI_WIFI)
		if (!strcmp(redirectUrl, "wlwps.htm")) 		
			vwlan_idx = 1;				
		else
			vwlan_idx = 0;	
#endif		
...
int wlP2PScanTbl(request *wp, int argc, char **argv)
{
	int nBytesSent=0, i;
	BssDscr *pBss;	
	char tmpBuf[100], tmpBuf2[100], p2pTypeStr[5];
...
//printf("\r\n pStatus->number=[%d],__[%s-%u]\r\n",pStatus->number,__FILE__,__LINE__);
	...
	for (i=0; i<pStatus->number && pStatus->number!=0xff; i++) 
	{
		...
		snprintf(tmpBuf2, 200, ("%02x%02x%02x%02x%02x%02x"),
			pBss->p2paddress[0], pBss->p2paddress[1], pBss->p2paddress[2],
			pBss->p2paddress[3], pBss->p2paddress[4], pBss->p2paddress[5]);
