...
#ifdef ACTIONTEC_WCB
    if(strstr(req->request_uri, ".htm") || strstr(req->request_uri, ".cgi")
        || strstr(req->request_uri, ".js") || strstr(req->request_uri, ".css"))
    {        
        req_write(req, "Cache-Control: private, no-cache, must-revalidate" CRLF);
        req_write(req, "Expires: 0" CRLF);     
    }
    else
    {
        req_write(req, "Cache-Control: max-age=19184000000" CRLF);       
    }
#else
...
#ifdef ACTIONTEC_MULTIACCESS
void send_redirect_cookie(request * req, const char *url, unsigned int session_id)
{
	char buff[200] = {0};
	char cookie_buf[300] = {0};
	...
	tmp = gmtime(&t);
	if (tmp != NULL) {
		if (strftime(gmt_time, sizeof(gmt_time), "%a, %d-%b-%Y %H:%M:%S GMT", tmp)) {	
			ktrace("gmtime1: %s", gmt_time);
		}
	}
	
	t = time(NULL) + SESSION_TIME;
    tmp = gmtime(&t);
	
    if (tmp != NULL) {
		if (strftime(gmt_time, sizeof(gmt_time), "%a, %d-%b-%Y %H:%M:%S GMT", tmp)) {	
			ktrace("gmtime2: %s", gmt_time);		 				
		}           
    }
#endif

	sprintf(cookie_buf, "Set-Cookie: %s%u", SESSION_KEY, session_id);	
	
	//ktrace("SESSION_TIME: %d", SESSION_TIME);
	//printf("redirect_cookie: %s\n", cookie_buf);
	
	req_write(req, cookie_buf);
