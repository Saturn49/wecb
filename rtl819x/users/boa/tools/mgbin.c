...
static void showHelp(void)
{
	...
	printf("%s Ver %s.\n\n", copyright, version);
	printf("usage: mgbin [-s] [-c] [-a] -o outputfile -f output_act_file bootcode config webpages linux root\n");
...
static int build_act_bin(const char *input, const char *bootFile, const char *output)
{
	FW_HEADER_T fw_hdr;
	...
	memcpy(fw_hdr.signature, ALL_HEADER, SIGNATURE_LEN);
	snprintf((char *)&fw_hdr.version, SOFTWARE_VERSION_MAX, "%s", fwVersion);
	fw_hdr.offset = WORD_SWAP(sizeof(fw_hdr));
	fw_hdr.hw_version = WORD_SWAP(AEI_BOARD_ID);
	...
	{
#ifdef WIN32
		fh_in = open(bootFile, O_RDONLY|O_BINARY);
#else
		fh_in = open(bootFile, O_RDONLY,S_IRUSR|S_IWUSR|S_IRGRP|S_IWGRP|S_IROTH|S_IWOTH);
#endif
		if(fh_in < 0)
		{
...
int main(int argc, char** argv)
{
	...
	const char *outbootFile = NULL;
#else
	char outFile[80]={0};
	char binFile[80]={0};
	char bootFile[80]={0};
	char outbootFile[80]={0};
#endif
	TYPE_T type;
	...
	while (argNum < argc) {
		if ( !strcmp(argv[argNum], "-o") ) {
			...
			if (++argNum >= argc)
				break;
#ifdef COMPACT_FILENAME_BUFFER
			bootFile = argv[argNum];
#else
			sscanf(argv[argNum], "%s", bootFile);
#endif
		}
			...
			if (++argNum >= argc)
				break;
#ifdef COMPACT_FILENAME_BUFFER
			outbootFile = argv[argNum];
#else
			sscanf(argv[argNum], "%s", outbootFile);
#endif
		}
...
//		allHeader.startAddr = DWORD_SWAP(image_num);
	...
	if(bootFile && bootFile[0] != '\0')
	{
		build_act_bin(outFile, bootFile, outbootFile);
	}

	build_act_bin(outFile, NULL, binFile);
