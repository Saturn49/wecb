			 ...
			 const char * const msg)
{
    ...
    if (level >= filter_level)
        fprintf(stderr, "[%s] %s %s %s\n", timestr, area, _xmpp_log_level_name[level], msg);
}
...
#ifdef AEI_XMPP
#define XMPP_DEBUG 1
#define XMPP_DEBUG_FILE "/tmp/xmpp_debug"
#define XMPP_DEBUG_BACKUP_FILE "/tmp/xmpp_debug0"
#define XMPP_DEBUG_FILE_LENGTH 1024 * 32

/** Write a log message to a debug file
 */
void write_log(char *buf)
{
...
#if !defined(XMPP_DEBUG)
  return;
#endif

  if(stat(XMPP_DEBUG_FILE, &sz) == 0)
  {
     if(sz.st_size >= XMPP_DEBUG_FILE_LENGTH)
     {
       memset(tmpdate, 0, sizeof(tmpdate));
       sprintf(tmpdate, "cp -f %s %s", XMPP_DEBUG_FILE, XMPP_DEBUG_BACKUP_FILE);
       system(tmpdate);
       unlink(XMPP_DEBUG_FILE); 
     }
  }
  ...
  memset(tmpdate, 0, sizeof(tmpdate));
  strftime(tmpdate, sizeof(tmpdate), "%d %b %Y %H:%M:%S", tmp);
  fp = fopen("/tmp/xmpp_debug", "ab+");
  if (!fp)
      return;
  //fprintf(fp, "\r\n%d.  %s\r\n", num, tmpdate);
  fprintf(fp, "%s\r\n", buf);
