...
static int http_da_post_header(struct soap *soap, const char *key, const char *val)
{
...
#ifdef AEI_TR69
  static char digest_realm[256]={0};
  static char *saveqop = NULL;
#endif
  ...
  if (soap->authrealm)
      snprintf(digest_realm, sizeof(digest_realm), "%s", soap->authrealm);
  else  {
      soap->authrealm = digest_realm;
  }
#endif
    ...
    if (data->qop && !soap_tag_cmp(data->qop, "*auth-int*"))
    {
      ...
      qop = saveqop;
      if (saveqop && !soap_tag_cmp(saveqop, "*auth-int*")) {
          soap_s2hex(soap, (unsigned char*)data->digest, entityHAhex, 16);
      }
    }
#else
