...
#if defined ( SUPPORT_BHR1 )
///TODO, added by Zhu.HongLiang for BHR1
static uint16_t htons (uint16_t s)
{
	return s;
}

static uint32_t htonl(uint32_t s)
{
	return s;
}
...
static uint32_t ntohl(uint32_t s)
{
	return s;
}

#endif /// defined ( SUPPORT_BHR1 )
...
#ifndef PALM_1
#ifdef AEI_TR69
static const char *soap_decode_nochange(char*, size_t, const char*, const char*);
#endif
...
const char*
SOAP_FMAC2
soap_decode_key_nochange(char *buf, size_t len, const char *val)
{ return soap_decode_nochange(buf, len, val, "=,;");
}
#endif
SOAP_FMAC1
const char*
...
const char*
SOAP_FMAC2
soap_decode_val_nochange(char *buf, size_t len, const char *val)
{ if (*val != '=')
  { *buf = '\0';
    return val;
  }
  return soap_decode_nochange(buf, len, val + 1, ",;");
}
#endif
...
#ifdef AEI_TR69
static const char*
soap_decode_nochange(char *buf, size_t len, const char *val, const char *sep)
{ const char *s;
  char *t = buf;
  for (s = val; *s; s++)
    if (*s != ' ' && *s != '\t' && !strchr(sep, *s))
...
{ register int c;
  ...
  while ((c = *s++) && --n > 0)
#ifdef AEI_TR69    /* no change cookie content */
{ if (c > ' ' && c < 128)
    *t++ = c;
  else
    break;
...
{ struct soap_cookie *p = NULL, *q;
  ...
  while (*s)
#ifdef AEI_TR69
  { s = soap_decode_key_nochange(tmp, sizeof(tmp), s);
#else
  { s = soap_decode_key(tmp, sizeof(tmp), s);
#endif
    if (!soap_tag_cmp(tmp, "$Version"))
#ifdef AEI_TR69
    { if ((s = soap_decode_val_nochange(tmp, sizeof(tmp), s)))
#else
...
#ifdef AEI_TR69
    { s = soap_decode_val_nochange(tmp, sizeof(tmp), s);
#else
...
#ifdef SOAP_DEBUG
#ifdef AEI_TR69
  soap_set_test_logfile(soap, "/tmp/TEST.log");
  soap_set_sent_logfile(soap, "/tmp/SENT.log");
  soap_set_recv_logfile(soap, "/tmp/RECV.log");
#else
...
#ifdef AEI_TR69
    if (soap->status!=204  && soap_get0(soap) == (int)EOF )
#else
...
void
SOAP_FMAC2
soap_digest_done(struct soap *soap)
{
    soap_done(soap);
