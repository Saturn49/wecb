...
#define EXPAT_ROUND_UP(n, sz) (((n) + ((sz) - 1)) & ~((sz) - 1))
	...
	*nextPtr = s;
	...
	if (nextPtr) {
	  /* Need to guarantee that:
	     tag->buf + EXPAT_ROUND_UP(tag->rawNameLength, sizeof(XML_Char)) <= tag->bufEnd - sizeof(XML_Char) */
	  if (tag->rawNameLength + (int)(sizeof(XML_Char) - 1) + (int)sizeof(XML_Char) > tag->bufEnd - tag->buf) {
	    int bufSize = tag->rawNameLength * 4;
	    bufSize = EXPAT_ROUND_UP(bufSize, sizeof(XML_Char));
	...
	if (startElementHandler) {
	  ...
	  for (;;) {
	    ...
	    int bufSize;
	    if (nextPtr)
	      toPtr = (XML_Char *)(tag->buf + EXPAT_ROUND_UP(tag->rawNameLength, sizeof(XML_Char)));
	    else
