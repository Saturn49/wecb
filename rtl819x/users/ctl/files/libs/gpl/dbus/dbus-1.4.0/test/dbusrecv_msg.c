...
int dbusrecv_reply(DBusConnection *connection, DBusMessage *message, void *reply_msg,int reply_len)
{
	...
	if (!dbus_message_append_args (reply,
	                  DBUS_TYPE_ARRAY,DBUS_TYPE_BYTE, &reply_msg,reply_len,
	                  DBUS_TYPE_INVALID))
	{
	    printf("%s: No memory, dbus_message_append_args\n", __FILE__);
	    goto EXIT;
...
int dbusrecv_reply_simple(DBusConnection *connection, DBusMessage *message, int ret)
{
	...
	do {
		...
		reply = dbus_message_new_method_return (message);
		if (NULL == reply) {
			printf( "No memory, dbus_message_new_method_return\n" );
			break;
		}
		if (!dbus_message_append_args (reply,
					...
					DBUS_TYPE_INVALID))
		{
			printf( "No memory, dbus_message_append_args\n" );
			break;
		}
		if (!dbus_connection_send (connection, reply, NULL))
		{
			printf( "No memory, dbus_connection_send\n" );
			break;
		}
	} while(0);
