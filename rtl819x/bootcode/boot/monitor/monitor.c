...
#ifdef AEI_WECB
    int CmdCheckImg(int argc, char* argv[]);
    int CmdSwapImg(int argc, char *argv[]);
#endif
...
#ifdef AEI_WECB
    { "AEICHECK", 0, CmdCheckImg, "AEICHECK: check SW Version"},
    { "AEISWAP", 0, CmdSwapImg, "AEISWAP: swap boot/back image"}
#endif
#endif 
};
...
// CID 29763: Wrong sizeof argument (SIZEOF_MISMATCH)
// Doesn't need memset argv[0]
#else
...
#ifdef AEI_COVERITY_FIX
    // CID 29561: Missing return statement (MISSING_RETURN)
    return 0;
#endif
}
...
int CmdCfn(int argc, char* argv[])
{
...
#ifdef AEI_COVERITY_FIX
    // CID 29806: Uninitialized scalar variable (UNINIT)
    else
    {
        return FALSE;
    }
	...
	jump();	
#ifdef AEI_COVERITY_FIX
    // CID 29558: Missing return statement (MISSING_RETURN)
    return 0;
#endif
}
...
#undef FLASH_WRITE_BYTE //4096
        ...
        //---------------------------------------------------------------------------
#ifdef AEI_COVERITY_FIX
    // CID 29560: Missing return statement (MISSING_RETURN)
    return 0;
#endif
}
...
#ifdef AEI_COVERITY_FIX
// CID 29762: Wrong sizeof argument (SIZEOF_MISMATCH)
#else
				memset(argv[0],0,sizeof(argv[0]));
#endif
				break;
...
#ifdef AEI_COVERITY_FIX
    // CID 29559: Missing return statement (MISSING_RETURN)
    return 0;
#endif
};
#endif
...
static int aei_read_flash_data(AEI_FLASH_DATA_T *data, unsigned int burn_offset)
{
    int ret = 0;
    int bank;
    ...
    case 0:
        bank = 0;
        break;
    case CONFIG_RTL_FLASH_DUAL_IMAGE_OFFSET:
        bank = 1;
        break;
    default:
        return -1;
    ...
    ret = flashread(data, CONFIG_AEI_FLASH_DATA_START + bank * AEI_FLASH_DATA_SIZE, sizeof(AEI_FLASH_DATA_T));
    if(ret != sizeof(AEI_FLASH_DATA_T))
    {
        prom_printf("Error: read flash_data error\n");
        return -1;
    }

    if((memcmp(data->signature, AEI_FLASH_SIGNATURE, SIG_LEN) != 0))
    {
        // prom_printf("Error: haven't found flash_data\n");
        return -1;
    }

    if((aei_check_sum_calc(data, sizeof(AEI_FLASH_DATA_T)) != 0))
    {
        prom_printf("Error: check flash_data error\n");
        return -1;
    }
...
static int aei_flash_check_sum(unsigned long addr, unsigned long len)
{
    int i;
...
static int aei_find_image(IMG_HEADER_T *pHeader, unsigned long offset, unsigned long start,
    unsigned long end, unsigned long step, unsigned long *addr)
{
    ...
    for(i = start; i <= end; i += step)
    {
        ...
        rAddr = (unsigned long)FLASH_BASE + offset + i;

        for(j = 0; j < sizeof(IMG_HEADER_T); j+=2, word_ptr++)
            *word_ptr = rtl_inw(rAddr + j);

        if(!memcmp(pHeader->signature, FW_SIGNATURE_WITH_ROOT, SIG_LEN))
        {
            ret = RET_FW_IMG;
            break;
        }
        else if(!memcmp(pHeader->signature, ROOT_SIGNATURE, SIG_LEN))
        {
            ret = RET_ROOT_IMG;
            break;
        }
        else if(!memcmp(pHeader->signature, WEB_SIGNATURE, SIG_LEN))
        {
            ret = RET_WEB_IMG;
            break;
        }
...
static int aei_check_image(IMG_HEADER_T *pHeader, unsigned long offset, unsigned long start,
    unsigned long end, unsigned long step, unsigned long *retAddr)
{
    unsigned long Addr;
    ...
    ret = aei_find_image(pHeader, offset, start, end, step, &Addr);
    if(ret == RET_NO_IMG){
        prom_printf("Haven't found image at 0x%x to 0x%x\n", offset + start, offset + end);
        goto DONE;
    }

    if(ret == RET_ROOT_IMG)
    {
        ret2 = aei_flash_check_sum(Addr + sizeof(IMG_HEADER_T), pHeader->len);
    }
    else
    {
        ret2 = aei_flash_check_sum(Addr + sizeof(IMG_HEADER_T), pHeader->len);
    ...
    if(ret2 == RET_CHECKSUM_ERR)
    {
        ret = ret2;
        prom_printf("bootbank checksum error\n");
    }
    else
    {
        if(retAddr != NULL)
...
int CmdCheckImg(int argc, char* argv[])
{
    IMG_HEADER_T bank1Header, bank2Header, tmpHeader;
    unsigned long retAddr;
    unsigned long bank1Mark = 0, bank2Mark = 0;
    AEI_FLASH_DATA_T data;
    int i;
    int ret;

    // Bank1 kernel
    ret = aei_check_image(&bank1Header, 0, CONFIG_LINUX_IMAGE_OFFSET_START,
        CONFIG_LINUX_IMAGE_OFFSET_END, CONFIG_LINUX_IMAGE_OFFSET_STEP, &retAddr);
    if(ret != RET_FW_IMG)
    {
        prom_printf("Error: Check bank1 kernel fail\n");
    }
    else
    {
        prom_printf("Found bank1 kenrel at 0x%x, ", retAddr - FLASH_BASE);
        bank1Mark = header_to_mark(ret, &bank1Header);

        ret = aei_read_flash_data(&data, 0);
        if(ret)
        {
            prom_printf("Can't get SW version\n");
        }
        else
        {
            prom_printf("SW version: %s\n", data.version);
        }
    }

#if 0
// rootfs and web haven't save the image header to flash, so can't check it
    // Bank1 rootfs
    ret = aei_check_image(&tmpHeader, 0, CONFIG_ROOT_IMAGE_OFFSET_START,
        CONFIG_ROOT_IMAGE_OFFSET_END, CONFIG_ROOT_IMAGE_OFFSET_STEP, NULL);
    if(ret != RET_ROOT_IMG)
    {
        prom_printf("Error: Check bank1 rootfs fail\n");
    }
#endif

    // Bank2 kernel
    ret = aei_check_image(&bank2Header, CONFIG_RTL_FLASH_DUAL_IMAGE_OFFSET, CONFIG_LINUX_IMAGE_OFFSET_START,
        CONFIG_LINUX_IMAGE_OFFSET_END, CONFIG_LINUX_IMAGE_OFFSET_STEP, &retAddr);
    if(ret != RET_FW_IMG)
    {
        prom_printf("Error: Check bank2 kernel fail\n");
    }
    else
    {
        prom_printf("Found bank2 kenrel at 0x%x, ", retAddr - FLASH_BASE);
        bank2Mark = header_to_mark(ret, &bank2Header);

        ret = aei_read_flash_data(&data, CONFIG_RTL_FLASH_DUAL_IMAGE_OFFSET);
        if(ret)
        {
            prom_printf("Can't get SW Version\n");
        }
...
#if 0
    // Bank2 root
    ret = aei_check_image(&tmpHeader, CONFIG_RTL_FLASH_DUAL_IMAGE_OFFSET, CONFIG_ROOT_IMAGE_OFFSET_START,
        CONFIG_ROOT_IMAGE_OFFSET_END, CONFIG_ROOT_IMAGE_OFFSET_STEP, NULL);
    if(ret != RET_ROOT_IMG)
    {
        prom_printf("Error: Check bank2 rootfs fail\n");
    }
#endif

    prom_printf("Default bootup %s\n", bank1Mark > bank2Mark ? "bank1" : "bank2");
...
/*
    Describe: Get The current kernel HW Version, then decide the LED behavior

    Return:
        Success: Return the current kernel HW version
        Fail: Return -1
*/
int AEIGetHWVersion(void)
{
    IMG_HEADER_T bank1Header, bank2Header;
    unsigned long bank1Addr = 0, bank2Addr = 0;
    unsigned long bank1Mark = 0, bank2Mark = 0;
    unsigned long burnAddr = 0, burnoffset = 0;
    AEI_FLASH_DATA_T data;
    int ret;

    memset(&data, 0, sizeof(AEI_FLASH_DATA_T));
    memset(&bank1Header, 0, sizeof(IMG_HEADER_T));
    memset(&bank2Header, 0, sizeof(IMG_HEADER_T));

    ret = aei_find_image(&bank1Header, 0, CONFIG_LINUX_IMAGE_OFFSET_START,
        CONFIG_LINUX_IMAGE_OFFSET_END, CONFIG_LINUX_IMAGE_OFFSET_STEP, &bank1Addr);
    if(ret != RET_FW_IMG)
    {
        prom_printf("Haven't found bank1 kernel image\n");
        bank1Mark = 0;
    }
    else
    {
    ...
    ret = aei_find_image(&bank2Header, CONFIG_RTL_FLASH_DUAL_IMAGE_OFFSET, CONFIG_LINUX_IMAGE_OFFSET_START,
        CONFIG_LINUX_IMAGE_OFFSET_END, CONFIG_LINUX_IMAGE_OFFSET_STEP, &bank2Addr);
    if(ret != RET_FW_IMG)
    {
        prom_printf("Haven't found bank2 kernel image\n");
        bank2Mark = 0;
    }
    else
    {
    ...
    {
        burnoffset = CONFIG_RTL_FLASH_DUAL_IMAGE_OFFSET;
    }

    ret = aei_read_flash_data(&data, burnoffset);
    if(ret < 0)
    {
        prom_printf("Can't get HW version\n");
        return -1;
    }

    return data.hw_version;
}
...
int CmdSwapImg(int argc, char *argv[])
{
    ...
    unsigned long burnAddr, burnoffset;
    AEI_FLASH_DATA_T data;
    ...
    if(ret != RET_FW_IMG)
    {
        prom_printf("Haven't found bank1 kernel image, can't swap Image\n");
        return 0;
    }
    bank1Mark = header_to_mark(ret, &bank1Header);
    ...
    if(ret != RET_FW_IMG)
    {
        prom_printf("Haven't found bank2 kernel image, can't swap Image\n");
        return 0;
    }
	bank2Mark = header_to_mark(ret, &bank2Header);
    ...
    if(burnAddr + sizeof(IMG_HEADER_T) > spi_flash_info[0].chip_size)
        {
        if(spi_flw_image_mio_8198(0,burnAddr, burnHeader, spi_flash_info[0].chip_size-(burnAddr))&&
            spi_flw_image_mio_8198(1,0, burnHeader+spi_flash_info[0].chip_size-(burnAddr), burnAddr+sizeof(IMG_HEADER_T)-spi_flash_info[0].chip_size))
            ret = sizeof(IMG_HEADER_T);
        }
    else
    {
        if(spi_flw_image_mio_8198(0,burnAddr, burnHeader, sizeof(IMG_HEADER_T)))
            ret = sizeof(IMG_HEADER_T);
    }
#else
    ret = spi_flw_image(0, burnAddr, burnHeader, sizeof(IMG_HEADER_T));
#endif
#else
    ret = flashwrite(burnAddr, burnHeader, sizeof(IMG_HEADER_T));
#endif
    if(ret == sizeof(IMG_HEADER_T))
    {

        prom_printf("Swap to %s bootup, ", bank1Mark > bank2Mark ? "bank2" : "bank1");
