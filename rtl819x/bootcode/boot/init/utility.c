...
#ifdef AEI_WECB
int check_web_image(unsigned long addr)
{
    // Read header, heck signature and checksum
    int i;
    ...
    word_ptr = (unsigned short *)&tmpHeader;
    for (i=0; i<sizeof(IMG_HEADER_T); i+=2, word_ptr++)
    {
        *word_ptr = rtl_inw(addr + i);
    }

    if ( memcmp(tmpHeader.signature, WEB_SIGNATURE, SIG_LEN))
    {
        prom_printf("no web image signature at %X!\n",addr-FLASH_BASE);
        return 0;
    }

    length = tmpHeader.len;
...
#if defined(NEED_CHKSUM)
        tmp_sum += rtl_inb(addr + sizeof(IMG_HEADER_T) + i);
#endif
    }
    ...
    if ( tmp_sum )
    {
        prom_printf("web image checksum error at %X!\n",addr-FLASH_BASE);
        return 0;
    }
#endif
    return 1;
...
// Check web image
            ...
            if(ret)
            {
                ret = check_web_image((unsigned long)FLASH_BASE+CONFIG_WEB_IMAGE_OFFSET+bank_offset);
            }
#endif
#endif
	}
