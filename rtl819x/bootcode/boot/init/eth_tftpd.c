...
// CID 29656: Out-of-bounds write (OVERRUN_STATIC)
// CID 29597: Out-of-bounds write (OVERRUN)
...
static int aei_write_flash_data(AEI_FLASH_DATA_T *data, unsigned int burn_offset)
{
    int ret = 0;
    int bank = 0;
    unsigned long burnAddr = 0;
    unsigned long burnLen = sizeof(AEI_FLASH_DATA_T);
    ...
    burnAddr = CONFIG_AEI_FLASH_DATA_START + bank * AEI_FLASH_DATA_SIZE;

#ifdef CONFIG_SPI_FLASH
#ifdef SUPPORT_SPI_MIO_8198_8196C
    if(burnAddr + burnLen > spi_flash_info[0].chip_size)
        {
        if(spi_flw_image_mio_8198(0,burnAddr, data, spi_flash_info[0].chip_size-(burnAddr))&&
            spi_flw_image_mio_8198(1,0, data+spi_flash_info[0].chip_size-(burnAddr), burnAddr+burnLen-spi_flash_info[0].chip_size))
            ret = burnLen;
        }
    else
        if(spi_flw_image_mio_8198(0,burnAddr, data, burnLen))
            ret = burnLen;
#else
    ret = spi_flw_image(0, burnAddr, data, burnLen);
#endif
#else
    ret = flashwrite(burnAddr, data, burnLen);
#endif
    if(ret != sizeof(AEI_FLASH_DATA_T))
    {
        prom_printf("Error: write flash_data error\n");
        return -1;
    }
...
void checkAutoFlashing(unsigned long startAddr, int len)
{
    ...
    FW_HEADER_T *all_header;
    unsigned long tmp_offset = 0;
    int have_aei_data = 0;
    int write_aei_data = 0;

    memset(&aei_data, 0, sizeof(AEI_FLASH_DATA_T));

    if(memcmp(((char *)startAddr + head_offset), ALL_HEADER, SIG_LEN) == 0)
    {
        all_header = (FW_HEADER_T *)((char *)startAddr + head_offset);
        memcpy(&aei_data.signature, AEI_FLASH_SIGNATURE, SIG_LEN);
        aei_data.len = sizeof(AEI_FLASH_DATA_T);
        memcpy(&aei_data.version, all_header->version, SOFTWARE_VERSION_MAX);
        aei_data.hw_version = all_header->hw_version;

        head_offset += sizeof(FW_HEADER_T);
        have_aei_data = 1;
    }

    // check burn address, confirm image will not recover bootcoder
    tmp_offset = head_offset;
    while( (tmp_offset + sizeof(IMG_HEADER_T)) <  len)
    {
        memcpy(&Header, ((char *)startAddr + tmp_offset), sizeof(IMG_HEADER_T));

        if((Header.burnAddr < CONFIG_BOOT_CODE_SIZE) && memcmp(Header.signature, BOOT_SIGNATURE, SIG_LEN))
        {
            printf("This image will recover bootcoder, reject upgrade\n");
            return;
        }

        tmp_offset += Header.len + sizeof(IMG_HEADER_T);
    }
#endif
	...
	while( (head_offset + sizeof(IMG_HEADER_T)) <  len){
		...
		else if(!memcmp(Header.signature, WEB_SIGNATURE, SIG_LEN))
		{
			burn_offset = sel_burnbank_offset();
			prom_printf("burn_offset = %x !\n",burn_offset);
		}
...
int trueorfaulse = 0;
    ...
    if(write_aei_data)
    {
        if(aei_write_flash_data(&aei_data, burn_offset) < 0)
        {
            return;
        }
        if(have_aei_data)
