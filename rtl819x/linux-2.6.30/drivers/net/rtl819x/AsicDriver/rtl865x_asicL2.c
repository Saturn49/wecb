...
#if 0											
											5, 	/* Queue-Packet=Based Flow Control turn off Threshold = 0x05 */
											6	/* Queue-Packet=Based Flow Control turn on Threshold =0x06 */
#else
											0xf, 	/* Queue-Packet=Based Flow Control turn off Threshold = 0x0f */
											0x3f	/* Queue-Packet=Based Flow Control turn on Threshold =0x3f */
#endif
										},
										{
											1, 		/* default: Bandwidth Control Include/exclude Preamble & IFG */
...
/* refer to  rtl8651_getAsicL2Table() and rtl8651_setAsicL2Table */
int32 rtl819x_cmpUpdL2Age(uint32 row, uint32 column, ether_addr_t *mac) 
{
	rtl865xc_tblAsic_l2Table_t   entry;
	unsigned char mac2[6];
 
	if((row >= RTL8651_L2TBL_ROW))
		return FAILED;

	_rtl8651_readAsicEntry(TYPE_L2_SWITCH_TABLE, row<<2 | column, &entry);

	if ((entry.agingTime == 0 && entry.isStatic == 0 ) || (entry.isStatic == 1))
		return FAILED;
	
	mac2[0] = entry.mac47_40;
	mac2[1] = entry.mac39_24 >> 8;
	mac2[2] = entry.mac39_24 & 0xff;
	mac2[3] = entry.mac23_8 >> 8;
	mac2[4] = entry.mac23_8 & 0xff;
	mac2[5] = row ^ mac2[0] ^ mac2[1] ^ mac2[2] ^ mac2[3] ^ mac2[4]  ^(fidHashTable[entry.fid]);

	if (memcmp(mac2, (const void *)mac, 6) == 0) {
		entry.agingTime = 3;
...
/* refer to rtl865x_Lookup_fdb_entry() */
int32 rtl819x_update_fdb_age(ether_addr_t *mac)
{
	uint32 rowIdx;
	uint32 colIdx;

	rowIdx = rtl8651_filterDbIndex(mac, RTL_LAN_FID);
	colIdx = 0;
	
	for(colIdx=0; colIdx<RTL8651_L2TBL_COLUMN; colIdx++) 
	{
		rtl819x_cmpUpdL2Age(rowIdx, colIdx, mac);
	}
	return SUCCESS;
}
#endif
...
//						| ((FRC_P0_EEE_100|EN_P0_TX_EEE|EN_P0_RX_EEE) << (port * 5));
	...
	*/
	...
	#define GMII_PIN_MUX 0xf00
       #if defined(CONFIG_8198_PORT5_GMII) || defined(CONFIG_8198_PORT5_RGMII) || defined(CONFIG_AEI_MOCA_ENABLE)
           REG32(PIN_MUX_SEL)= REG32(PIN_MUX_SEL)&(~(GMII_PIN_MUX));
       #endif
       ...
       WRITE_MEM32(PCRP4, READ_MEM32(PCRP4) | ((4<<ExtPHYID_OFFSET)|EnablePHYIf|MacSwReset) ); /* Jumbo Frame */
		...
		REG32(P5GMIICR) = REG32(P5GMIICR) | 0x40; //Conf_done=1
                REG32(PCRP5) = 0 | (0x5<<ExtPHYID_OFFSET) |      //JSW@20100309:For external 8211BN GMII ,PHYID must be 5
                               	EnForceMode| ForceLink|ForceSpeed1000M |ForceDuplex | PauseFlowControlDtxErx | /* PauseFlowControlEtxErx: force: disable TX, enable RX */
                                MIIcfg_RXER | EnablePHYIf | MacSwReset;
...
#if defined(AEI_WECB) && defined(AEI_ETH_FC_DISABLE)
    WRITE_MEM32(PCRP0, READ_MEM32(PCRP0) & (~PauseFlowControlEtxErx)); /* Disable Flow Control on Port 0 */
    WRITE_MEM32(PCRP1, READ_MEM32(PCRP1) & (~PauseFlowControlEtxErx)); /* Disable Flow Control on Port 1 */
#endif
...
int32 rtl8651_getAsicEthernetPHYReg(uint32 phyId, uint32 regId, uint32 *rData)
{
	...
	do { status = READ_MEM32( MDCIOSR ); } while ( ( status & MDC_STATUS ) != 0 ); // wait until command complete
