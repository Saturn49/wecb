...
#if defined(AEI_WECB) && defined(AEI_ETH_PORT_UNPLUG_NOTIFY) 
#include <net/netlink.h>

static struct sock *net_port_unplug_netlink_sk __read_mostly = NULL;
static void net_port_unplug_netlink_rcv(struct sock *sk, int len); 
int __init net_port_unplug_netlink_init();//struct net *net);
void net_port_unplug_netlink_fini(void);
int net_port_unplug_netlink_broadcast(struct net* net, unsigned int status);
...
#if defined(AEI_WECB) && defined(AEI_ETH_PORT_UNPLUG_NOTIFY)
    if(is_all_ethernet_ports_down())
    {
        net_port_unplug_netlink_broadcast(dev_net(cp->dev), status);
    }
#endif
}
#endif
...
int __init net_port_unplug_netlink_init()
{
    printk(KERN_ERR"%s enter\n", __func__);
    if(net_port_unplug_netlink_sk != NULL)
        return 0;

    net_port_unplug_netlink_sk = netlink_kernel_create(NULL, NETLINNK_NET_PORT_UNPLUG, 0,
            net_port_unplug_netlink_rcv, NULL, THIS_MODULE);
    if(!net_port_unplug_netlink_sk)
    {
...
void net_port_unplug_netlink_fini(void)
{
    if(net_port_unplug_netlink_sk != NULL)
    {
        netlink_kernel_release(net_port_unplug_netlink_sk);
    }
}

int is_ethernet_port_on(int port_num)
{
    struct lan_port_status port_status;
    int32 rc = 0;
    int ret = 0;

    rc = rtl819x_get_port_status(port_num, &port_status);
    if(rc < 0)
    {
        printk(KERN_ERR"%s get port %d status error\n", __func__, port_num);
        return ret;
    }
    ...
    printk(KERN_ERR"port %d status %d\n", port_num, port_status.link);
    return ret;

}

int is_all_ethernet_ports_down()
{
    int ret = 0;
    if( !is_ethernet_port_on(0) && !is_ethernet_port_on(1))
    {
        ret = 1;
    }
...
int net_port_unplug_netlink_broadcast(struct net *net, unsigned int status)
{
    ...
    skb_out = alloc_skb(NLMSG_SPACE(sizeof(status)), GFP_KERNEL);
    if(!skb_out)
    {
        printk(KERN_ERR "alloc skb error\n");
        return -1;  
    }
    nlh = NLMSG_PUT(skb_out, 0, 0, 0x111, sizeof(status));
    memcpy(NLMSG_DATA(nlh), &status, sizeof(status)); 

    nlh->nlmsg_pid = 0;
    nlh->nlmsg_flags = 0;
    NETLINK_CB(skb_out).pid = 0;
    NETLINK_CB(skb_out).dst_group = 1;
    ret = netlink_broadcast(net_port_unplug_netlink_sk, skb_out, 0, 1, GFP_KERNEL);

    printk("%s done\n", __func__);
    return ret;
nlmsg_failure:
    kfree_skb(skb_out);
    printk(KERN_ERR"%s nlmsg_failure\n", __func__);
    return -EINVAL; 
}
#endif
