...
static struct rtl_clientEntry* rtl_getClientEntry(uint32 ipVersion, struct rtl_groupEntry* groupEntry,  uint32 *clientAddr)
{
...
void rtl_delete_module_clientEntry(uint32 ipVersion,uint32 moduleIndex,uint32 * groupAddress,uint32 *clientAddr)
{
	struct rtl_groupEntry *groupEntry=NULL;
	struct rtl_clientEntry* clientEntry=NULL;
	int i=0;
	for(i=0; i<MAX_MCAST_MODULE_NUM; i++)
	{
		if(i==moduleIndex)
			continue;
		if(rtl_mCastModuleArray[i].enableSnooping){
			groupEntry = rtl_searchGroupEntry(i, ipVersion, groupAddress);
			if(groupEntry){
				clientEntry=rtl_getClientEntry( ipVersion, groupEntry,  clientAddr);
				rtl_deleteClientEntry(groupEntry,clientEntry);
			}
		}
	}
			...
			while(groupEntryPtr!=NULL)
			{
				...
				{	
					...
					if(((1<<clientEntry->portNum) & portInfo->linkPortMask)==0)
					{
						...
						clientAddr[0]=clientEntry->clientAddr[0];
						ipVersion=IP_VERSION4;
						
						rtl_deleteClientEntry(groupEntryPtr,clientEntry);
						rtl_delete_module_clientEntry(ipVersion,moduleIndex,groupAddress,clientAddr);
						clearFlag=TRUE;
					}
			...
			while(groupEntryPtr!=NULL)
			{
				...
				{	
					...
					if(((1<<clientEntry->portNum) & portInfo->linkPortMask)==0)
					{
						...
						clientAddr[0]=clientEntry->clientAddr[0];
						clientAddr[1]=clientEntry->clientAddr[1];
						clientAddr[2]=clientEntry->clientAddr[2];
						clientAddr[3]=clientEntry->clientAddr[3];
