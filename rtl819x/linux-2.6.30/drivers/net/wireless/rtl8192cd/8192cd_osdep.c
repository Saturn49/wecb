...
static int rtl8192cd_init_sw(struct rtl8192cd_priv *priv)
{
	...
	{
#ifdef AEI_WECB
        //don't erase LED_Timer, because it is initialized in rtl8192cd_init_one for slow blinking during wifi initializing
        offset = (unsigned long)(&((struct priv_shared_info*)0)->LED_tx_cnt_log);
#else   // zero all data members below (including) LED_Timer of share_info
...
#ifdef WIFI_WMM
#ifdef CONFIG_RTL_92D_SUPPORT
		if (GET_CHIP_VER(priv) == VERSION_8192D && priv->pmib->dot11OperationEntry.wifi_specific)
...
//		priv->ps_ctrl = 0x11;
	...
	{
#ifdef AEI_WECB
        //using software LED for all customers
        //using software LED with LEDTYPE_SW_CUSTOMER
        LED_TYPE = LEDTYPE_SW_CUSTOMER;
#endif
		// for HW/SW LED
...
#ifdef CONFIG_RTL_VLAN_8021Q
	extern unsigned short cached_wlan_vid;
		cached_wlan_vid = 0;
...
#ifdef AEI_WECB
    init_wlan_led(priv, LED_MODE_BLINK, SLOW_BLINK_INTERVAL_ON, SLOW_BLINK_INTERVAL_OFF);
#endif
...
inline int check_vlan_member(unsigned int member, int index){
	int flag = 0;
	if(linux_vlan_enable){
		/*make sure wlan interface is support by linux vlan*/
		if(index>=WLAN0_MASK_BIT && index<=WLAN1_VXD_MASK_BIT){
		/*make sure interface is in the same vlan group*/
			if(member & (1<<index))
				flag = 1;
		}
	}
...
struct net_device *get_shortcut_dev(unsigned char *da)
{
	...
	if(linux_vlan_enable){
		vid= *((unsigned short *)(da+(ETH_ALEN<<1)+2));
		vid &=0xfff;
		member=vlan_ctl_p->group[vid].memberMask;
	}
	else{
#endif
		...
		if (wlan_device[i].priv && netif_running(wlan_device[i].priv->dev)) {
			...
			if (priv
				...
				if (!priv->pmib->dot11OperationEntry.disable_brsc) {
                    ...
                    if(check_vlan_member(member,index))
#endif
                    {
...
#endif						
					{	/* Make sure it must have some packets go theough bridge module before shortcut */
						#ifdef WDS
...
#if defined(CONFIG_RTL_VLAN_8021Q)
							index=priv->pvap_priv[j]->dev->vlan_member_map;
							if(check_vlan_member(member,index))
#endif
							{
...
#if defined(CONFIG_RTL_VLAN_8021Q)
					index=vxd_priv->dev->vlan_member_map;
					if(check_vlan_member(member,index))
#endif
					{
...
void clear_shortcut_cache(void)
{
	...
	{
		int i;
		extern struct cached_br_info cached_eth_info[CACHE_ETH_DEV_MAX];
		for(i=0;i<CACHE_ETH_DEV_MAX;i++){
			cached_eth_info[i].vid=0;
			cached_eth_info[i].dev=NULL;
		}
	}
#endif
}
