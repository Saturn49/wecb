...
#define AEI_WLAN_TX_OFFLOAD_GATING_PPS_DEFAULT        30000
#define AEI_WLAN_TX_OFFLOAD_QUEUE_LENGTH_DEFAULT      128
#define AEI_WLAN_TX_OFFLOAD_QUEUE_DURATION_DEFAULT    60
#define AEI_WLAN_TX_OFFLOAD_QUEUE_TIMER_DEFAULT       40

static int aei_wlan_tx_offload_on = 0;
static int aei_wlan_tx_pps = 0;
static struct proc_dir_entry *aei_wlan_tx_offload_proc=NULL;

#ifdef CONFIG_AEI_WLAN_TX_OFFLOAD_PROC_PARAM
#ifdef _INCLUDE_PROC_FS_
#include <linux/proc_fs.h>
#endif

#define AEI_WLAN_TX_OFFLOAD_GATING_PPS             aei_wlan_tx_offload_gating_pps
#define AEI_WLAN_TX_OFFLOAD_QUEUE_LENGTH           aei_wlan_tx_offload_queue_length
#define AEI_WLAN_TX_OFFLOAD_QUEUE_DURATION         aei_wlan_tx_offload_queue_duration
#define AEI_WLAN_TX_OFFLOAD_QUEUE_TIMER            aei_wlan_tx_offload_queue_timer

static int aei_wlan_tx_offload_gating_pps=AEI_WLAN_TX_OFFLOAD_GATING_PPS_DEFAULT;
static int aei_wlan_tx_offload_queue_length=AEI_WLAN_TX_OFFLOAD_QUEUE_LENGTH_DEFAULT;
static int aei_wlan_tx_offload_queue_duration=AEI_WLAN_TX_OFFLOAD_QUEUE_DURATION_DEFAULT;
static int aei_wlan_tx_offload_queue_timer=AEI_WLAN_TX_OFFLOAD_QUEUE_TIMER_DEFAULT;

#ifdef _INCLUDE_PROC_FS_
static int aei_wlan_tx_offload_read(struct file *file, const char *buffer,
              unsigned long count, void *data)
{
	printk("gating pps      %d\n"
	       ...
	       "tx_pps          %d\n"
	       "offload on      %d\n",
	       aei_wlan_tx_offload_gating_pps,
	       aei_wlan_tx_offload_queue_length,
	       aei_wlan_tx_offload_queue_duration,
	       aei_wlan_tx_offload_queue_timer,
	       aei_wlan_tx_pps,
	       aei_wlan_tx_offload_on);

	return count;
}

static int aei_wlan_tx_offload_write(struct file *file, const char *buffer,
              unsigned long count, void *data)
{
	printk("usage: echo gating_pps queue_length queue_duration queue_timer > /proc/aei_wlan_tx_offload\n");

	printk("BEFORE CHANGE: gating pps %d, queue length %d, queue duration %d, queue timer %d\n",
	       aei_wlan_tx_offload_gating_pps,
	       aei_wlan_tx_offload_queue_length,
	       aei_wlan_tx_offload_queue_duration,
	       aei_wlan_tx_offload_queue_timer);
	...
	printk("AFTER CHANGE: gating pps %d, queue length %d, queue duration %d, queue timer %d\n",
	       aei_wlan_tx_offload_gating_pps,
	       aei_wlan_tx_offload_queue_length,
	       aei_wlan_tx_offload_queue_duration,
	       aei_wlan_tx_offload_queue_timer);
...
static int __init aei_wlan_tx_init_proc(void)
{
	aei_wlan_tx_offload_proc = create_proc_entry("aei_wlan_tx_offload", 0, NULL);
	if (aei_wlan_tx_offload_proc) {
		aei_wlan_tx_offload_proc->read_proc = aei_wlan_tx_offload_read;
		aei_wlan_tx_offload_proc->write_proc = aei_wlan_tx_offload_write;
	}
	return 1;
}

static void __exit aei_wlan_tx_exit_proc(void)
{
	if (aei_wlan_tx_offload_proc) {
		remove_proc_entry("aei_wlan_tx_offload", aei_wlan_tx_offload_proc);
	}
	return;
}
module_init(aei_wlan_tx_init_proc);
module_exit(aei_wlan_tx_exit_proc);
#endif

#else
#define AEI_WLAN_TX_OFFLOAD_GATING_PPS			AEI_WLAN_TX_OFFLOAD_GATING_PPS_DEFAULT
#define AEI_WLAN_TX_OFFLOAD_QUEUE_LENGTH		AEI_WLAN_TX_OFFLOAD_QUEUE_LENGTH_DEFAULT
#define AEI_WLAN_TX_OFFLOAD_QUEUE_DURATION		AEI_WLAN_TX_OFFLOAD_QUEUE_DURATION_DEFAULT
#define AEI_WLAN_TX_OFFLOAD_QUEUE_TIMER			AEI_WLAN_TX_OFFLOAD_QUEUE_TIMER_DEFAULT
#endif

static inline int aei_wlan_tx_need_offload(void)
{
	return (aei_wlan_tx_offload_on > 0);
}

void aei_wlan_tx_offload_tx_cnt_report(int difftime,int pktcnt)
{
	/* difftime always be 100 more or less,
	 * in this case, unknown error happened.
	 */
	if( (difftime > HZ+20) || (difftime < HZ-20) ) {
		return;
	}

	aei_wlan_tx_pps = pktcnt;

	if ( pktcnt > AEI_WLAN_TX_OFFLOAD_GATING_PPS ) {
		aei_wlan_tx_offload_on = AEI_WLAN_TX_OFFLOAD_QUEUE_DURATION;
	}
	/* In WLAN TX offload mode */
	else if (aei_wlan_tx_offload_on > 1) {
		aei_wlan_tx_offload_on--;
	}
	else if (aei_wlan_tx_offload_on == 1) {
		aei_wlan_tx_offload_on = 0;
	}
	return;
}

static int aei_wlan_tx_get_pps(void)
{
	return aei_wlan_tx_pps;
}
#endif
...
#endif						
					if(txcfg->pstat && (txcfg->pstat->rssi > 25))
...
__MIPS16
__IRAM_IN_865X
static int aei_wlan_tx_offload_sw_enqueue(struct rtl8192cd_priv *priv, struct sk_buff *skb, struct stat_info	*pstat)
{
	int q_len, pri, q_num, *pempty, aggnum;
	...
	if (q_len >= AEI_WLAN_TX_OFFLOAD_QUEUE_LENGTH) {
		...
		*pempty = 0;
		...
		if (!timer_pending(ptimer)) {
			...
			mod_timer(ptimer, jiffies + AEI_WLAN_TX_OFFLOAD_QUEUE_TIMER);
		}
	}
...
#ifdef AEI_REMOVE_VLAN_ON_WIFI_TX
    // RTL patch for #79916,
    // remove VLAN headers before sending out to any wireless interface,
    // for any packets with VLAN header
    if (*((unsigned short *)(skb->data+MACADDRLEN*2)) == __constant_htons(ETH_P_8021Q)) {
        int i;
        for (i=0; i<6; i++)
            *((unsigned short *)(skb->data+MACADDRLEN*2+2-i*2)) = *((unsigned short *)(skb->data+MACADDRLEN*2-2-i*2));
...
#ifdef CONFIG_AEI_WLAN_TX_OFFLOAD
	if ( pstat && aei_wlan_tx_need_offload() ) {
		return aei_wlan_tx_offload_sw_enqueue(priv,skb,pstat);
	}
#endif
