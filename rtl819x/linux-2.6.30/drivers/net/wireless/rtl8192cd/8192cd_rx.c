...
__inline__ struct net_device *get_eth_cached_dev(unsigned char *da)
{
	...
	if(linux_vlan_enable){
		...
		cached_wlan_vid = vid & 0xfff;
		for(i=0;i<CACHE_ETH_DEV_MAX;i++){
			/*both vid and mac should be the same*/
			if(cached_eth_info[i].vid==cached_wlan_vid
				&& !memcmp(da, cached_eth_info[i].mac, MACADDRLEN)){
				return cached_eth_info[i].dev;
			}
		}
		return NULL;
	}	
...
#ifdef CONFIG_RTL_VLAN_8021Q 
	if(linux_vlan_enable){			
		struct net_device *cached_eth_dev=NULL;	
		if (!(pskb->data[0] & 0x01)){
			/*get cached net_device when mac and vid are both matched*/
			if(((cached_eth_dev = get_eth_cached_dev(pskb->data)) != NULL)
				&& netif_running(cached_eth_dev)){
				pskb->dev = cached_eth_dev;
			#if !defined(__LINUX_2_6__) || defined(CONFIG_COMPAT_NET_DEV_OPS)
				cached_eth_dev->hard_start_xmit(pskb, cached_eth_dev);
			#else
				cached_eth_dev->netdev_ops->ndo_start_xmit(pskb,cached_eth_dev);
			#endif
				return;
			}		
		}
