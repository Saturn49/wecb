...
static struct sock *msg_sock = NULL;

static void msg_netlink_recv(struct sk_buff *skb)
{
	printk("%s() LINE %d, recv netlink message\n", __func__, __LINE__);
	return;
}

static int msg_netlink_init(void)
{
	msg_sock = netlink_kernel_create(NULL, NETLINK_MONITOR, 0, msg_netlink_recv, NULL, THIS_MODULE);
	if (!msg_sock)
	{
...
static void msg_netlink_exit(void)
{
	sock_release(msg_sock->sk_socket);

	return;
}

int msg_netlink_broadcast(void *buf, __u16 type, int size)
{
	...
	nlh = (struct nlmsghdr *) skb_put(skb, NLMSG_SPACE(size));
	// nlh->nlmsg_type = NLMSG_DONE;

	NETLINK_CB(skb).pid = 0;  /* from kernel */
	NETLINK_CB(skb).dst_group = 1;

	return netlink_broadcast(msg_sock, skb, 0, 1, GFP_KERNEL);
}
